...
#######TOPIC SELECTION##########
        ...
        for z in range(len(cityList)):
            Multiquery=mongo.db.Agenda.count_documents({'$and':[ {'$text': { "$search": chosen}}, {"MeetingType":" City Council "}, {"City": cityList[z]}, { 'Date':{'$lte':today, '$gte':oneyearBefore}}]})
            if Multiquery == 0:
                continue
            else:
...
#######TOPIC SELECTION##########
        ...
        if request.method == 'GET':
            agendaa = mongo.db.Agenda.find({'$and':[ {'$text': { "$search": 'oil'}}, { 'Date':{'$lte':twoweekAhead, '$gte':threemonthBefore}}]}).sort('Date').sort('City')
